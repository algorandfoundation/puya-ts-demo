...
(async function () {
  ...
  const acct = await algokit.getLocalNetDispenserAccount(
    algodClient,
    kmdClient,
  );
  ...
  const { transaction } = await simple.create.createApplication({});

  const { appId, appAddress } = await simple.appClient.getAppReference();
  console.log(
    `Created app ${appId} with address ${appAddress} in tx ${transaction.txID()}`,
  );
  ...
  console.log(`counter == 3?: ${counter === 3n}`);

  const addResult = await simple.add({ a: 123, b: 456 });
