...
async function getBoxValue(boxIndex: number, arc: string) {
  ...
  return valueType.decode(
    await arc75.appClient.getBoxValue(getBoxRef(boxIndex, arc).name),
  );
}
...
describe("ARC75", function () {
  beforeAll(async function () {
    ...
    await algokit.ensureFunded(
      {
        accountToFund: acct,
        minSpendingBalance: algokit.microAlgos(1_000_000),
        suppressLog: true,
      },
      algodClient,
      kmdClient,
    );
    senderAddr = acct.addr.toString();
  ...
  test("removeWithSet", async function () {
    ...
    const preBalance = (await algodClient.accountInformation(senderAddr).do())
      .amount;

    await setApps(0, id, [44]);

    const balance = (await algodClient.accountInformation(senderAddr).do())
      .amount;
