...
const optInLsigTeal = readFileSync(
  `contracts/lsig-with-app/out/OptInLsig.lsig.teal`,
  "utf8",
);
...
describe("Lsig With App", () => {
  ...
  beforeAll(async () => {
    ...
    const { confirmation } = await algokit.sendTransaction(
      { transaction: asaCreateTxn, from: testAccount },
      algod,
    );
  ...
  test("Program signed by bob", async () => {
    ...
    const lsigAccount = await getLsigAccount(
      algod,
      BigInt(appId),
      optInLsigTeal,
    );
  ...
  test("verifyCreator and opt in", async () => {
    ...
    lsigAccount.sigkey = algosdk.decodeAddress(bob.addr.toString()).publicKey;
    ...
    await appClient.verifyCreator(
      ...
      {
        optIn: { transaction: optInTxn, signer: lsigAccount },
      },
      {
