...
describe("ZK Whitelist", () => {
  ...
  it("should be able to add address to whitelist", () => {
    ...
    const proof = new arc4.DynamicArray<arc4.Address>(
      new arc4.Address(Bytes(new Uint8Array(Array(32).fill(0)))),
    );

    const dummyVerifierApp = ctx.any.application({
      appLogs: [ABI_RETURN_VALUE_LOG_PREFIX.concat(Bytes.fromHex("80"))],
    });
  ...
  it("returns error message if proof verification fails", () => {
    ...
    const dummyVerifierApp = ctx.any.application({
      appLogs: [ABI_RETURN_VALUE_LOG_PREFIX.concat(Bytes(""))],
    });
    ctx.setTemplateVar("VERIFIER_APP_ID", dummyVerifierApp.id);
  ...
  it("returns true if address is already in whitelist", () => {
    ...
    const dummyAccount = ctx.any.account({
      optedApplications: [ctx.ledger.getApplicationForContract(contract)],
    });
