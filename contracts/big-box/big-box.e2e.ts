...
import appSpec from "./out/BigBox.arc32.json";
...
describe("Big Box", () => {
  ...
  beforeAll(async () => {
    sender = (await algokit.getDispenserAccount(algodClient, kmdClient))
      .account;
    data = fs.readFileSync("contracts/big-box/TEAL.pdf");
  ...
  test("startUpload", async () => {
    ...
    const mbrPayment = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
      sender: sender.addr,
      receiver: (await appClient.getAppReference()).appAddress,
      amount: totalCost,
  ...
  test("upload", async () => {
    ...
    const boxPromises = boxData.map(async (box, boxIndex) => {
      ...
      boxes.push({
        appIndex: 0,
        name: new Uint8Array(Buffer.from("TEAL.pdf")),
      });
    ...
    const boxValuePromises = boxData.map(async (_, boxIndex) =>
      appClient.getBoxValue(algosdk.encodeUint64(boxIndex)),
    );

    const boxValues = await Promise.all(boxValuePromises);

    boxValues.forEach((val, i) => {
      expect(Buffer.from(val).toString("hex")).toEqual(
        boxData[i].toString("hex"),
      );
    });

    expect(Buffer.concat(boxValues).toString("hex")).toEqual(
      data.toString("hex"),
    );
  });
});
