...
async function main() {
  ...
  const sender = await algokit.getLocalNetDispenserAccount(
    algodClient,
    kmdClient,
  );
  ...
  console.log(
    `root match: ${root === expectedRoot}, root: ${root}, expected: ${expectedRoot}`,
  );

  const recordsToAppend = await getRecords("contracts/merkle/append.csv");
  ...
  const recordsToVerify = await getRecords(
    "contracts/merkle/verify_appends.csv",
  );
  ...
  const recordsToUpdate = await getRecords("contracts/merkle/update.csv");
  for (const record of recordsToUpdate) {
    ...
    console.log(
      `Updated ${record[0]} to ${record[1]} in ${result.transaction.txID()}`,
    );
  }

  const recordsToVerifyAgain = await getRecords(
    "contracts/merkle/verify_updates.csv",
  );
